...
public class HomeController {
	...
	public String home(@RequestParam(name = "date", required = false) String date, Model model) {
		...
		// 合計カロリー取得
		model.addAttribute("totalKcal", intakeSvc.calcTotalCal(USER_ID, targetDate));
		// getDailyRecordsで対象日の履歴を取得
		model.addAttribute("targetDateAteRecords", intakeSvc.getDailyRecords(USER_ID, targetDate));
	...
	public String intakeDetail(@RequestParam("intakeId") long intakeId, Model model) {
		// IDから詳細情報を取得
		Map<String, Object> map = intakeSvc.getIntakeDetail(USER_ID, intakeId);
		if(map == null) {
	...
	public String eatMaker(Model model) {
		// ユーザーIDに紐づくメーカー一覧を取得
		List<Map<String, Object>> makers = intakeSvc.getMakerList(USER_ID);
	...
	public String eatFoods(@RequestParam("makerId") long makerId, Model model) {
		// 表示するメーカー名を取得
		String makerName = intakeSvc.getMakerName(USER_ID, makerId);

		// ユーザーIDと選択したメーカーに紐づく食品一覧を取得
		List<Map<String, Object>> foods = intakeSvc.getFoodList(USER_ID, makerId);
	...
	public String eatFlavors(@RequestParam("foodId") long foodId,
		...
		Map<String, Object> headerInfo = intakeSvc.getHeaderInfo(USER_ID, foodId);

		// ユーザーIDと食品IDに紐づく味一覧を取得
		List<Map<String, Object>> flavorList = intakeSvc.getFlavorList(USER_ID, foodId);
	...
	public String delete(@RequestParam("intakeId") long intakeId) {
		intakeSvc.delIntake(USER_ID, intakeId);
		return "redirect:/";
	}
	...
	public String makerCreate(@RequestParam("makerName") String makerName, RedirectAttributes ra) {
		// メーカー重複チェック
		if(intakeSvc.chkDepliMaker(USER_ID, makerName) == -1) {
			ra.addFlashAttribute("msg", "同じメーカーが既に登録されています");
			return "redirect:/makers/new";
		}

		intakeSvc.insFoodMaker(USER_ID, makerName);
	...
	public String foodCreateAndNext(@RequestParam("makerId") long makerId,
		...
		// 重複チェック
		if(intakeSvc.chkDepliFood(USER_ID, foodName, makerId) == -1) {
			ra.addFlashAttribute("msg", "同じメーカー内に同名の食品が既に登録されています");
			return "redirect:/foods/new";
		}
		// 食品情報登録
		long foodId = intakeSvc.insFood(USER_ID, foodName, makerId);
	...
	public String flavorCreate(
		...
		// 重複チェック
		if(intakeSvc.chkDepliFlavor(USER_ID, flavorName, foodId) == -1) {
			ra.addFlashAttribute("msg", "同じ味の栄養情報が既に登録されています");
			return "redirect:/flavors/new";
		}

		// 栄養情報登録
		intakeSvc.insFlavor(USER_ID, flavorName, foodId, calorie, protein, lipid, carbo, salt);
	...
	public String intakeUpdate(
		...
		intakeSvc.updIntake(USER_ID, intakeId, eatenDate, eatenTime);
