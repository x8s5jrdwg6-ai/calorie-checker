...
--------------------------------------*/
	...
	public int calcTotalCal(String userId, LocalDate eatenDate) {
		String sql = """
				...
				FROM intake i
				JOIN flavor fl ON fl.flavor_id = i.flavor_id
	...
	public Map<String, Object> getIntakeDetail(String userId, long intakeId) {
		String sql = """
			    ...
			    JOIN flavor fl ON fl.flavor_id = i.flavor_id
			    JOIN food f ON f.food_id = fl.food_id
			    JOIN food_maker m ON m.food_maker_id = f.food_maker_id
			    WHERE i.intake_id = ?
		...
		Map<String, Object> intake = jdbc.queryForObject(
			    ...
			    (rs, rowNum) -> {
			        ...
			        map.put("flavorName", rs.getString("flavor_name"));
...
--------------------------------------*/
	 ...
	 *	@return	登録件数（通常は1）
	 */
	public int insIntake(String userId, long flavorId, LocalDate eatenDate, LocalTime eatenTime) {
		String sql = """
		         INSERT INTO intake (regist_user_id, flavor_id, eaten_date, eaten_time, qty)
		         VALUES (?, ?, ?, ? ,1)
		     """;
		// 登録件数を返す（通常 1）。失敗時は例外が投げられることが多い
		return jdbc.update(sql, userId, flavorId, eatenDate, eatenTime);
	}
	
	public Map<String, Object> getHeaderInfo (String userId, long foodId) {
		String sql = """
				SELECT f.food_id, f.food_name, m.food_maker_id, m.maker_name
				FROM food f
				JOIN food_maker m ON m.food_maker_id = f.food_maker_id
		...
		Map<String, Object> headerInfo = jdbc.queryForObject(
			    ...
			    (rs, rowNum) -> {
			        ...
			        map.put("foodName", rs.getString("food_name"));
			        map.put("foodMakerId", rs.getString("food_maker_id"));
			        map.put("makerName", rs.getString("maker_name"));
			        return map;
			    }
			);
	...
	public List<Map<String, Object>> getMakerList(String userId){
		String sql = """
				SELECT food_maker_id, maker_name
				FROM food_maker
	...
	public List<Map<String, Object>> getFlavorList(String userId, long foodId){
		String sql = """
				SELECT flavor_id, flavor_name, calorie, protein, lipid, carbo, salt
				FROM flavor
	...
	public List<Map<String, Object>> getFoodList(String userId, long makerId){
		String sql = """
				...
				FROM food
				WHERE regist_user_id = ? AND food_maker_id = ?
				ORDER BY food_name
			""";
...
--------------------------------------*/
	...
	public long getRegistFoodId(String userId, String foodName,long makerId) {
		String sql = "SELECT food_id FROM food WHERE regist_user_id=? AND food_maker_id=? AND food_name=?";
...
--------------------------------------*/
	 ...
	 *	@return	登録件数（通常は1）
	 */
	public int insFlavor(String userId, String flavorName, long foodId, int calorie, Double protein, Double lipid, Double carbo, Double salt) {
		String sql = "INSERT INTO flavor (flavor_name, food_id, calorie, protein, lipid, carbo, salt, regist_user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
